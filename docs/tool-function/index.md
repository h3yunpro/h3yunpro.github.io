---
url: 'https://h3yunpro.github.io/docs/tool-function/index.md'
---
...
传入业务对象和要触发计算公式的控件编码即可。主表、子表业务对象均可传入，但子表控件编码只能传子表内控件编码，不能传“子表编码.子表内控件编码”。计算完成后，结果会直接赋值到业务对象对应控件，无需接收返回值再次赋值。

注意事项：

* 主表控件的计算公式只能引用主表上的其他控件，调用时传入主表业务对象。
* 子表控件的计算公式只能引用当前子表上的其他控件，调用时传入对应的子表业务对象。
* 如果计算公式存在主表与子表之间的跨表引用，则不适合使用此方法，请自行通过代码实现计算。

```cs
public static void CalcPropertyComputationRule(H3.DataModel.BizObject bo, string propertyCode)
{
    if(bo == null)
    {
        return;
    }
    if(!bo.Schema.PropertyExist(propertyCode))
    {
        return;
    }
    H3.DataModel.PropertySchema proSchema = bo.Schema.GetProperty(propertyCode);
    if(string.IsNullOrWhiteSpace(proSchema.ComputationRule))
    {
        return;
    }
    string comRule = proSchema.ComputationRule.Replace(bo.Schema.SchemaCode + ".", "");
    bo[propertyCode] = bo.CalcExpression(comRule);
}
```
...
```cs
//主表控件：传入主表业务对象和主表控件编码
CalcPropertyComputationRule(mainBo, "F0000001");

//子表控件：传入子表业务对象和子表内控件编码，不要传“子表编码.子表内控件编码”
CalcPropertyComputationRule(detailBo, "F0000002");
```
